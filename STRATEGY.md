...
> **Atlantic City high-limit: 8-deck shoe · dealer peeks for blackjack · dealer STANDS on all 17s
> (S17) · blackjack pays 3:2 · double on any two cards · double after split (DAS) · split to 4 hands ·
> split aces get one card · late surrender · $100 table minimum · $10,000 bankroll (with re-buy).**

Rule changes move these numbers. 6:5 blackjacks, no DAS, no surrender, or a dealer who HITS soft
17 (H17, common outside Atlantic City) each shift the edge, and a few of them flip specific cells.
Verify the felt before you sit.

All EV figures below are **per unit bet**, generated by exact dynamic programming for this ruleset
(the same tables that power the in-app Coach) and spot-verified by multi-million-round Monte-Carlo
simulation. "EV −0.54" means you lose, on average, 54¢ of every $1 wagered on that decision.
...
The cliff is real: a dealer **6 busts ~42%**, a **7 only ~26%**. That 16-point drop between 6 and 7
...
you bust ~62% of the time outright. Both lines land at about −0.54 — *worse* than just handing back
...
|------|-----:|-----:|-----:|------|
| 16 vs 10 | −0.54 | −0.54 | −0.50 | **Surrender** |
| 16 vs 9  | −0.54 | −0.51 | −0.50 | **Surrender** (barely) |
| 15 vs 10 | −0.54 | −0.50 | −0.50 | **Surrender** (marginal — hit is a near-tie) |
| 16 vs A  | −0.67 | −0.52 | −0.50 | **Surrender** |
| 15 vs A  | −0.60 | −0.48 | −0.50 | **Play on** (S17 — hit beats surrender) |
| 17 vs A  | −0.48 | −0.58 | −0.50 | **Stand** (S17 — standing beats surrender) |

### The full late-surrender set (this S17 8-deck game)
...
That's the whole list under S17 — never surrender a soft hand, and never surrender 8,8 (splitting is
worth more everywhere). On **H17** tables (dealer hits soft 17, common outside Atlantic City) the
dealer wins slightly more often with an Ace up, which drags three more borderline hands below the
−0.50 line: those games add **15 vs A, 17 vs A, and 8,8 vs A**.

Surrender is checked **first** — before hit/stand/double/split — because it caps your loss at half.

---

## 3. Counting: how it works, and does it matter on a shoe game?
...
### "A big shoe, so the count is almost useless" — is that fair?
...
- Off the top the house has ~0.5%. Counting doesn't remove that on most hands; it tells you *when* to
  bet more and *when* the deck has swung. On an 8-deck shoe the true count only drifts to strongly
...
programming: build the dealer's final-total distribution (S17, with the peek — all values are
...
of Casino Blackjack* (Springer, 2nd ed. 2018)**. The infinite-deck computation matches shoe-game
composition-dependent values to a few thousandths of a bet — smaller than any decision boundary the
coach displays — and was cross-checked against our own full-shoe Monte-Carlo simulator (e.g. soft 18 vs 6
...
- **House edge:** a perfect-basic-strategy agent playing these exact rules over 2M seeded rounds
  returns **−0.49% without surrender and −0.41% with late surrender**, matching the published
  ~0.43% basic-strategy edge for Atlantic City rules (8-deck, S17, DAS, LS).
- **Surrender / odd-hand EVs:** the generated per-action EV tables (dynamic programming, S17 peek
  model) put every surrender cell in §2 below −0.50 played out, and were spot-verified against an
  independent 3M-round Monte-Carlo (agreement to 3 decimals).
- **Count integrity:** the Hi-Lo count sums to exactly **0** over a full shoe (the balanced-count
