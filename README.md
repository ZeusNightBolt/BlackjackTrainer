...
- **"What if?" counterfactual replay** — after every round (in Drill and Coach Me), the trainer replays your first decision against the **same shoe** with each alternative action and shows the realized outcome: stand on 15 v 6 and watch the dealer bust? It shows the hit would have busted *you*. Surrender a hand? It plays the hand out anyway (simulated — never counted) so you see what surrender saved or cost. Each line is framed as strategy vs. variance: "right call, worse result — don't let one draw retrain you."
