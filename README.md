...
- **Casino feel, phone-first** — a felt with a wooden rail and the classic printed arcs ("BLACKJACK PAYS 3 TO 2"), a betting circle that stacks your chips as you build the bet, animated card deals and hole-card flips, a confetti WIN flash, a win/lose glow on the felt, and on phones the action buttons dock to the bottom of the screen so Hit/Stand are always under your thumb. Add it to your iPhone home screen and it runs like an app (touch icon + standalone metas included). Bankroll, session stats, and settings persist in `localStorage`, so closing the tab doesn't reset your session.
